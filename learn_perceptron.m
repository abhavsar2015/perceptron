...
fprintf('%d',num_neg_examples);
num_pos_examples = size(pos_examples_nobias,1);
fprintf('%d',num_pos_examples);
num_err_history = [];
w_dist_history = [];
...
display (num_neg_examples);
num_pos_examples = size(pos_examples,1);
display (num_pos_examples);
...
for i=1:num_neg_examples
    this_case = neg_examples(i,:);
     ...
     if (activation >= 0) 
        %YOUR CODE HERE
       w=w+0.5*this_case.'*(0.0-activation);
     end
...
for i=1:num_pos_examples
    ...
    if (activation < 0)
        %YOUR CODE HERE
         w=w+0.5*this_case.'*(1-activation);
    end
